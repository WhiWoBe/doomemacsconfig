#+title: DOOM EMACS FROM SOURCE ON WSL AND CONFIGURATION
...
alternativelt you can download from the nearby GNU mirror
...
4. Check if installed via

   $ which emacs

For some reason i had to create symlinks to /usr/local/bin/emacs. Additionally i had to install some libs manually.
...
* General Information regarding the config
** General
